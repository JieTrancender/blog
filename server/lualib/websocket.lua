...
function ws.new(id, header, handler, conf, addr)
    ...
    local code, result, realIp = accept_connection(header, conf.check_origin, handler.check_origin_ok)
    if code then
    ...
    local realIp = header["x-forwarded-for"] or header["x-real-ip"]
    self.handler.on_open(self, realIp or addr)
